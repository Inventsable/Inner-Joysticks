...
function scanStringForSliderData(data) {
    if (/BEGIN\sSLIDER\sCODE/gm.test(data)) {
        var sliderControls = [];
        var sliderNameRX = /var\scontrl\s\=\sthisComp\.layer\(\"(.*)\"\)\;/gm;
        if (sliderNameRX.test(data)) {
            var sliderName = data.match(sliderNameRX)[1];
            var sliderControllerRX = /var\scontrlCurVal\d{1,}\s\=\scontrl\(\"(.*)\"\)\((\d{1,})\)/gm;
            if (sliderControllerRX.test(data)) {
                var controllerMatches = data.match(sliderControllerRX);
                ...
                for (var fx2 = 1; fx2 <= prop.numProperties; fx2++) {
                    ...
                    if (prop2.name == "Position" && !/text/i.test(prop.matchName)) {
                        if (prop2.expression) {
                            ...
                            var check = scanStringForSliderData(prop2.expression);
                            console.log(check);
                        }
                        else {
        ...
        for (var i = 1; i <= activeItem.layers.length; i++) {
            ...
            for (var fx = 1; fx <= layer.numProperties; fx++) {
                ...
                if (prop.name == "Effects") {
                    ...
                    var check = checkifJoystickBounds(prop);
                    if (check) {
                        if (config.hideJoysticks)
                            layer.enabled = false;
                    }
                        ...
                        if (nnnprop.numProperties) {
                            for (var nnnn = 1; nnnn <= nnnprop.numProperties; nnnn++) {
                                ...
                                if (nnnnprop.expression) {
                                    ...
                                    check = /\/{5}begin\slimit\scode\/{6}/gim.test(nnnnprop.expression);
...
function setJoystick(data) {
    ...
    var layer = activeItem.layer(data.name);
    if (layer) {
        var newpos = [data.x, data.y, 0];
...
function getActiveComp() {
    ...
    var selComp = app.project.selection.length === 1 &&
